...
#*******************************
# by Yin Dejiang, 20241009, yin.dj@qq.com
# The path of output!
# E.g., the following format!
# Out_path = '/home/ydj/test/results'
# Out_path = ""
# Input_path = ""
# Source_name = ""
# Obs_data = ""

def myexecute(cmd, output_file):
    print("'%s'" % cmd)
...
for dDM, dsubDM, dmspercall, downsamp, subcall, startDM in zip(dDMs, dsubDMs, dmspercalls, downsamps, subcalls, startDMs):
    ...
    for ii in range(subcall):
        ...
        if outsubs:
            ...
            myexecute("prepsubband -lodm %.2f -dmstep %.2f -numdms %d -downsamp %d -o %s %s" %
                      (loDM, dDM, dmspercall, datdownsamp, basename, subnames), 'prepsubband_commands.txt')
        else:
            ...
            #myexecute("cd %s && prepsubband -nobary -mask ./*.mask -ignorchan 680:810 -nsub %d -lodm %.2f -dmstep %.2f -numdms %d -downsamp %d -o %s %s" %
            #          (Out_path ,nsub, loDM, dDM, dmspercall, downsamp, basename, rawfiles), 'prepsubband_commands.txt')
            myexecute("prepsubband -nobary -ignorechan 680:810 -nsub %d -lodm %.2f -dmstep %.2f -numdms %d -downsamp %d " %
                      (nsub, loDM, dDM, dmspercall, downsamp), f'{basename}_de-DM_MOSS.comd')
"""
